...
RSpec.describe "Photo Facade" do
  describe 'Photo', :vcr do
    it 'makes an API call using the Unsplash gem' do
      ...
      expect(parse[:attributes][:table]).to have_key (:user)
      expect(parse[:attributes][:table][:user]).to be_a Hash
      expect(parse[:attributes][:table][:user]).to have_key (:name)
      expect(parse[:attributes][:table][:user][:name]).to be_a String
      expect(parse[:attributes][:table][:user]).to have_key (:links)
      expect(parse[:attributes][:table][:user][:links]).to be_a Hash
      expect(parse[:attributes][:table][:user][:links]).to have_key (:self)
      expect(parse[:attributes][:table][:user][:links][:self]).to be_a String
      expect(parse[:attributes][:table][:user][:links][:self]).to include "https://api.unsplash.com/users"
